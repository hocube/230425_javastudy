...
public class Ex02_Main extends JFrame {
	...
	public Ex02_Main() {
		...
		jta.setFont(new Font("굴림", Font.PLAIN, 13));
		...
		jb3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (custid.length() > 0) {
					...
					jta.setText("");
					if (idchk) {
						JOptionPane.showMessageDialog(getParent(), "같은 custid가 존재하지않습니다");
						ArrayList<Ex02_VO> list = Ex02_DAO.getInstance().getSelectAll();
						jta.append("\n\t\t회원 전체 정보\n\n");
						jta.append("\t번호\t이름\t주소\t전화번호\n");
						for (Ex02_VO k : list) {
						...
						if (result > 0) {
							JOptionPane.showMessageDialog(getParent(), "삭제성공");
							ArrayList<Ex02_VO> list = Ex02_DAO.getInstance().getSelectAll();
							jta.append("\n\t\t회원 전체 정보\n\n");
							jta.append("\t번호\t이름\t주소\t전화번호\n");
							for (Ex02_VO k : list) {
					...
					JOptionPane.showMessageDialog(getParent(), "custid 를 입력해주세요");
				}
				jtf1.setText("");
				jtf2.setText("");
				jtf3.setText("");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (msg.equals("검색")) {
					...
					if (custid.length() > 0) {
						...
						jta.setText("");
						if (idchk) {
							JOptionPane.showMessageDialog(getParent(), "custid가 존재하지않습니다");
						} else {
							Ex02_VO vo = new Ex02_VO();
							vo.setCustid(custid);
