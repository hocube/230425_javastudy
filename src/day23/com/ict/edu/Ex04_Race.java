	...
	public Ex04_Race() {
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				new Thread(new Runnable() {
					...
					public void run() {
						while (x1 < canvas.getWidth() - 140) {
							x1 += (int) (Math.random() * 10); // 0~9까지 돈다.
							try {
								Thread.sleep(40);
				...
				}, "1번 친구").start(); // while문 끝나면 스레드 이름 찍힘(1번 친구)
				...
				new Thread(new Runnable() {
					...
					public void run() {
						while (x2 < canvas.getWidth() - 140) {
							x2 += (int) (Math.random() * 10); // 0~9까지 돈다.
							try {
								Thread.sleep(40);
							} catch (InterruptedException e) {
								e.printStackTrace();
				...
				}, "2번 친구").start(); // while문 끝나면 스레드 이름 찍힘(1번 친구)
				...
				new Thread(new Runnable() {
					...
					public void run() {
						while (x3 < canvas.getWidth() - 140) {
							x3 += (int) (Math.random() * 10); // 0~9까지 돈다.
							try {
								Thread.sleep(40);
							} catch (InterruptedException e) {
								e.printStackTrace();
				...
				}, "3번 친구").start(); // while문 끝나면 스레드 이름 찍힘(1번 친구)
				...
				new Thread(new Runnable() {
					...
					public void run() {
						while (x4 < canvas.getWidth() - 140) {
							x4 += (int) (Math.random() * 10); // 0~9까지 돈다.
							try {
								Thread.sleep(40);
							} catch (InterruptedException e) {
								e.printStackTrace();
				...
				}, "4번 친구").start(); // while문 끝나면 스레드 이름 찍힘(1번 친구)
			}
		});
