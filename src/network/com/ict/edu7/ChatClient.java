...
public class ChatClient extends JFrame implements Runnable {
	...
	public ChatClient() {
		...
		pg = new JPanel();
		pg.setBorder(new EmptyBorder(10, 10, 10, 10)); // 테두리
		...
		JPanel jp1_1 = new JPanel(new FlowLayout(FlowLayout.RIGHT)); // 오른쪽 정렬
		...
		jsp.setViewportView(user_list); // 리스트가 많아지면 스크롤로 내려야 하니깐
		...
		jsp2.setViewportView(room_list); // 리스트가 많아지면 스크롤로 내려야 하니깐
	...
	private boolean connected() {
		boolean value = true;
		try {
			s = new Socket("192.168.31.168", 7779);
