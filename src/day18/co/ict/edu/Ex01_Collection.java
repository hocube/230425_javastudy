...
public class Ex01_Collection {
	...
	//  API : <T> = 객체 타입, <E> = 요소(Element) = 컬렉션 안에 존재하는 객체 하나를 뜻 함, 
	//		  Map형식:<K,V> = Key, Value 
	//				 1:1 대응, ★Key를 호출하면 Value가 나온다★
	// 	형식 : 컬렉션<제네릭>
	
	// 최상위 컬렉션은 Collection<E>, Map<K,V>
	// Collection<E>를 상속 받아서 실제로 사용하는 인터페이스들은 : Set<E>, List<E>, Queue<E>
	
	// Collection의 주요 메서드
	// add(E e) : boolean => 해당 컬렉션에 객체 추가. 성공하면 true, 실패하면 false
	//						 보통은 리턴을 받지 않는다. (set에서는 중복 검사 시 받는다)
	// addAll(Collection< ? extends E> c) : boolean
	//		  => 특정 컬렉션에 있는 모든 요소들을 다른 컬렉션에 추가
	// clear() : (반환형)void => 모든 요소 삭제
	...
	// 			   Arrays.asList() => 배열을 컬렉션(Array List)으로 만든다.
}
